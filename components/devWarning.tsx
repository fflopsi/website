...
export default function DevWarning() {
  return process.env.VERCEL_ENV !== 'production' ? (
    <>
      ...
      <p>The present version of this website is based on commit <Link href={`https://${process.env.VERCEL_GIT_PROVIDER}.com/${process.env.VERCEL_GIT_REPO_OWNER}/${process.env.VERCEL_GIT_REPO_SLUG}/commit/${process.env.VERCEL_GIT_COMMIT_SHA}`}>{process.env.VERCEL_GIT_COMMIT_SHA}</Link>.</p>
      <hr />
    </>
  ) : '';
}
